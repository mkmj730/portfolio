    ...
    <section id="projects" className="py-14 sm:py-20">
      ...
      <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
        {projects.map((p, idx) => {
          ...
          const cardClass =
            "rounded-2xl p-5 sm:p-6 flex flex-col gap-3 bg-white/60 dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10 hover:shadow-sm hover:ring-brand/30 transition";

          return p.slug ? (
            <Link key={idx} href={`/work/${p.slug}`} className={cardClass} aria-label={`${p.title} 상세 보기`}>
              {CardInner}
            </Link>
          ) : (
